...
class DNMS:
    ...
    def practice(self, win, i):
        ...
        for j in range(3):
            ...
            if choice_pos == 0:
                ...
                core.wait(0.7)
                win.flip()
            print('GET NEXT : ', dev.response_queue)
            print('SIZE QUEUE', dev.response_queue_size())
    ...
    def get_response(self):
        ...
        while not dev.has_response():
            ...
            print("ahahhaah gotya")
        response = dev.get_next_response()
        print("reps : ", response)
        dev.clear_response_queue()
        print("response queue after clear :", dev.response_queue)
        #resp = event.waitKeys(keyList=keys, timeStamped=rt_timer)

        if 'q' in str(response["key"]):
            self.quit_experiment()
        return str(response["key"]), response["time"]  # key and rt in milliseconds
